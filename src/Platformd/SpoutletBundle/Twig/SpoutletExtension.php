...
class SpoutletExtension extends \Twig_Extension
{
    ...
    public function __construct($bucketName, $giveawayManager, $linkableManager, $mediaExposer, $router, TokenStorageInterface $tokenStorage, $siteUtil, $translator, $userManager, $contentReportRepo, $siteRepo, $backgroundAdRepo, $localAuth, $secureUrlScheme, $flashUtil, KernelInterface $kernel)
    {
    ...
    public function SiteLink($linkType)
    {
        ...
        switch ($linkType) {
            case 'ALIENWARE_LINK':
                return $this->GetAlienwareLink($locale);
            case 'ALIENWARE_LINK_BOTTOM_RIGHT':
                return $this->GetAlienwareBottomRightLink($locale);
            case 'ALIENWARE_LINK_ADDRESS':
                return $this->GetAlienwareLinkAddress($locale);
            case 'FACEBOOK':
                return $this->GetFacebookLink($locale);
            case 'EVENTS':
                return $this->GetEventsLink($locale);
            case 'VIDEOS':
                return $this->GetVideosLink($locale);
            case 'WALLPAPERS':
                return $this->GetWallpapersLink($locale);
            case 'TWITTER':
                return $this->GetTwitterLink($locale);
            case 'USER_EVENT':
                return $this->GetUserEventLink($locale);
            case 'USER_GAME_ID':
                return $this->GetUserGameIdLink($locale);
            case 'USER_PROFILE':
                return $this->GetUserProfileLink($locale);
            case 'USER_GIVEAWAY':
                return $this->GetUserGiveawayLink($locale);
            case 'SOCIAL_MEDIA_STRIP':
                return $this->GetSocialMediaStripForHeaderAndFooter($locale);
            case 'PHOTOS':
                return $this->GetPhotosLink($locale);
            case 'CONTESTS':
                return $this->GetContestsLink($locale);
            case 'CONTESTS_IMAGE':
                return $this->GetContestsLink($locale, 'image');
            case 'CONTESTS_GROUP':
                return $this->GetContestsLink($locale, 'group');
            case 'GIVEAWAYS':
                return $this->GetGiveawaysLink($locale);
            case 'INSTAGRAM':
                return $this->GetInstagramLink($locale);
    ...
    private function GetAlienwareBottomRightLink($locale)
    {
        ...
        switch ($locale) {
            case 'ja':
                $text = 'ALIENWARE.JPへ移動する';
                break;
            case 'zh':
                $text = '需要一个牛逼的装备? 请看看 ALIENWARE';
                break;
            case 'es':
                $text = '¿Busca un equipo poderoso?<br />¡Encuéntrelo en Alienware!';
                break;
    ...
    private function GetAlienwareLinkAddress($locale)
    {
        ...
        switch ($locale) {
            case 'ja':
                return 'http://alienware.jp/';
            case 'zh':
                return 'http://alienware.com.cn/';
            case 'es':
                return 'http://www.alienware.com/mx/';
            case 'en_SG':
                return 'http://www.alienware.com.my/';
            case 'en_AU':
                return 'http://www.alienware.com.au/';
            case 'en_GB':
                return 'http://www1.euro.dell.com/content/topics/segtopic.aspx/alienware?c=uk&cs=ukdhs1&l=en&s=dhs&~ck=mn';
            case 'en_IN':
                return 'http://www.alienware.co.in/';
    ...
    private function GetSocialMediaStripForHeaderAndFooter($locale)
    {
        ...
        switch ($locale) {
            case 'zh':
                return $this->GetSocialMediaStripForHeaderAndFooterChina();
            default:
                return false;
        }
    }
    ...
    private function GetFacebookLink($locale)
    {
        ...
        switch ($locale) {
            case 'ja':
                return sprintf($format, 'DellJapan', $enAltText);
            case 'es':
                return sprintf($format, 'AlienwareLatinoamerica', $enAltText);
            case 'en_IN':
                return sprintf($format, 'alienwareindia', $enAltText);
            case 'en_US':
                return sprintf($format, $enLink, $enAltText);
            case 'en_GB':
                return sprintf($format, $enLink, $enAltText);
    ...
    private function GetTwitterLink($locale)
    {
        ...
        switch ($locale) {
            case 'ja':
                return sprintf($format, 'Alienware_JP', $enAltText);
            case 'es':
                return sprintf($format, 'AlienwareLatAm', $enAltText);
            case 'en_US':
                return sprintf($format, $enLink, $enAltText);
            case 'en_GB':
                return sprintf($format, $enLink, $enAltText);
    ...
    private function GetUserEventLink($locale)
    {
        ...
        switch ($locale) {
            case 'zh':
                return sprintf($format, '/china');
            case 'en_US':
                return sprintf($format, '');
            case 'en_SG':
                return sprintf($format, '/sg');
            case 'es':
                return sprintf($format, '/latam');
            case 'en_GB':
                return sprintf($format, '');
            case 'en_AU':
                return sprintf($format, '/anz');
            case 'en_IN':
    ...
    private function GetCevoCountryLookup($locale, $blankForNorthAmerica = true)
    {
        ...
        switch ($locale) {
            ...
            case 'en_IN':
                return 'in';
            case 'en_US':
                return $blankForNorthAmerica ? '' : 'na';
        }
    ...
    public function countrySpecific($key)
    {
        switch ($key) {
            case "alienware_link":
                ...
                break;
            default:
                throw new \InvalidArgumentException(sprintf('Unknown country-specific key "%s"', $key));
        }
